...
async function publishNpm({dir, config, execute = false, store, fetcher = fetch}) {
  ...
  if (m.branch !== "test") throw new Error("Self-contained npm COS publication currently supports test only");
  ...
  const plan = {mode: execute ? "execute" : "dry-run", component: m.component, version: m.version, upload: files.map(f => `${versionRoot}/${f}`)};
...
if (require.main === module) (async () => {
  ...
  if (args["--execute"]) {
    ...
    if (m.sourceRef !== source.ref) throw new Error("npm source ref does not match this repository's test branch");
  }
