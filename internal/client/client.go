...
func (c *Client) Do(ctx context.Context, req *Request) ([]byte, error) {
	...
	return c.doWithRetry(ctx, req.Service, req.Method, req.Path, u, headers, bodyBytes, contentType, req.BinaryResponse, req.OutputPath, req.SuppressSpaceHeader)
}
...
func (c *Client) doWithRetry(ctx context.Context, service, method, requestPath, urlStr string, headers map[string]string, body []byte, contentType string, binaryResp bool, outputPath string, suppressSpaceHeader bool) ([]byte, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		body, err := c.attempt(ctx, service, method, requestPath, urlStr, headers, body, contentType, binaryResp, outputPath, suppressSpaceHeader)
		if err == nil {
			return body, nil
		}
		lastErr = err
...
func (c *Client) attempt(ctx context.Context, service, method, requestPath, urlStr string, headers map[string]string, body []byte, contentType string, binaryResp bool, outputPath string, suppressSpaceHeader bool) ([]byte, error) { //nolint:gocyclo // HTTP attempt handles auth, headers, dry-run, binary, retries in one flow
	...
	if unsupportedErr := unsupportedAPIError(service, requestPath, resp.StatusCode, respBody); unsupportedErr != nil {
...
// unsupportedAPIError distinguishes a missing feature route from a normal
// resource-level 404. Supported APIs use the structured
// {"error":{"code":"..."}} envelope, so an unstructured 404 on a known feature
// path indicates an older gateway/backend where that API is not mounted.
func unsupportedAPIError(service, requestPath string, status int, body []byte) *output.ExitError {
	...
	switch {
	case isOctoProjectAPIPath(path):
		return output.ErrOctoProjectAPIUnsupported()
	case path == "/fleet/api/v1/workspaces" || strings.HasPrefix(path, "/fleet/api/v1/workspaces/"):
		return output.ErrWorkspaceAPIUnsupported()
	case service == "loop" || path == "/fleet/api/v1" || strings.HasPrefix(path, "/fleet/api/v1/"):
		return output.ErrLoopAPIUnsupported()
	default:
		return nil
	}
}

func isOctoProjectAPIPath(path string) bool {
