...
type Server struct {
	...
	credentialToken string // "" → resolve from env (stdio)
...
// there is one (notifications produce none).
...
func (s *Server) handleToolCall(ctx context.Context, req rpcRequest) rpcResponse {
	...
	case "call_op":
		...
		if err != nil {
			return newResultResponse(req.ID, jsonToolResult(map[string]any{"ok": false, "error": map[string]any{"type": "validation", "code": "INVALID_ARGUMENTS", "message": err.Error()}}, true))
		}
		s.mu.Lock()
		defer s.mu.Unlock()
